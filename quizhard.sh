...
updateScore() {
    ...
    local currentScore=$(awk -F '[:/]' '{print $2}' "$leaderboard")

    local score=$(("$currentScore" + "$correctAnswer"))

    local currentQuestions=$(awk -F '[:/]' '{print $3}' "$leaderboard")

    local questions=$(("$totalQuestions" + "$currentQuestions"))

    sed -i "/^$userName:/d" "$leaderboard"

    echo "$userName:$score/$questions" >> "$leaderboard"
